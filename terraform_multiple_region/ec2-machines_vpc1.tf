...
#  ami           = "${lookup(var.AmiLinux, "us-west-1")}"
  ami           = "${lookup(var.AmiLinux, "us-east-2")}"
  ...
  echo "\$conn = new mysqli('mydatabase.guba-terraform.test', 'root', 'secret', 'test');" >> /var/www/html/calldb.php
...
#  ami           = "${lookup(var.AmiLinux, "us-west-1")}"
  ...
  mysql -u root -psecret -e "INSERT INTO mytable (mycol) values ('gubagoo') ;" test
HEREDOC
}